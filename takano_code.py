...
def QMC(num_samples, num_thermalization, syst, flow, starting_Z, starting_F, expector, dz, dphi):
    ...
    for i in range(total_MC_steps):
        ...
        if i >= num_thermalization:
            ...
            sz = syst.observable(Z_prime,F_prime)
            ...
            tmp = list(itertools.chain(*zip(Z_prime.real.flatten(), Z_prime.imag.flatten())))
            print("Z_PRIME: ", end="")
            print(str(tmp).replace(' [', '').replace('[', '').replace(']', '').replace('\n', '').replace(',',''))
...
def main():
    ...
    theory, beta, coupling, ncfgs, ntherm, dz, dphi, Tflow = str(params[0]), float(params[1]), float(params[2]), int(params[3]), int(params[4]), float(params[5]), float(params[6]), float(params[7])   
 
    #print inputs  
    print("[theory, beta, coupling, ncfgs, ntherm, dz, dphi, Tflow] = [{}, {}, {}, {}, {}, {}, {}, {}]".format(params[0],params[1],params[2],params[3],params[4],params[5],params[6],params[7]))
